...
/** `resolveTenantDisplayName`: the join between scenario tenant refs and the seeder's namespaced display names. */
...
test("an unseeded tenant fails loudly, naming what was seeded", () => {
  // The pre-namespace literal must fail with a readable message, not a locator timeout.
  assert.throws(
