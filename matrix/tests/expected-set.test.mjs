...
//
// Expected-set canaries for the SEED rows. expected-set.ts and the scenario
// runner share one `satisfies()`, so a predicate bug moves gating and
// expectation together; these pin the EXACT expected-run list as the
// external witness. Re-derive deliberately with
//   cd tests/browser && npx tsx scripts/expected-set.ts ../../matrix/rows/<row>/capabilities.json
// Offline: no stack, no browser, no network.
...
const DEPLOYED_CORE_RUN = [
  ...
  "specs/settings.spec.ts :: backup-code-reuse-rejected",
...
for (const [row, expectedRun] of [
  ...
  test(`expected-set for ${row} is exactly the pinned list`, () => {
    ...
    // Run and skip must partition the collected tier-A set.
