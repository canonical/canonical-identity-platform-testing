...
export const model = {
  ...
  upstreamFindings: [
    "OBSERVED 2026-08-31, login-ui:stable (compose): the device-code page collapses the BFF's precise rejection to a generic error — a wrong user code makes PUT /api/device answer a raw HTTP 500 'Failed to accept user code' (the AcceptUserCode error branch, canonical/identity-platform-login-ui@197703c9 pkg/device/handlers.go — its NOT_FOUND_ERROR_DESC 'invalid, expired or already used' never reaches the wire), and /ui/device_code renders only 'Something went wrong, please try again' — the S-8 status/message-collapse class. MEASURED same day: NO rate limiting or lockout on user-code attempts anywhere in the chain — 6 wrong codes against one device_challenge and 4 across fresh flows all answer identically in ~12ms, while RFC 8628 §5.2 recommends rate-limiting exactly this surface (entropy mitigates: 8-char base62 codes, 10-minute lifespan). Covered: device-code-invalid-rejected pins the visible-error behaviour, not the wording — and stays deterministic BECAUSE no limiter exists (re-measure if one lands).",
