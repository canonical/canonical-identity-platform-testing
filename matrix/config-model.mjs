...
export const model = {
  ...
  dimensions: [
    ...
    },
    {
      ...
      effect:
        "Token shape seen by relying parties. The browser suite's access-token claim assertions decode JWTs; opaque rows assert on the ID token alone (access-token claims are declared null). No existing profile varies this. Opaque pairs ONLY with tenant_service=absent and hook_service=absent: both add-ons authenticate their admin APIs by parsing the bearer token as a JWT, so an opaque deployment cannot be administered or seeded (constraints opaque-tokens-lock-out-*).",
    },
  ],
  ...
  constraints: [
    ...
    {
      id: "opaque-tokens-lock-out-tenant-admin",
      forbid: { access_token: "opaque", tenant_service: "present" },
      reason:
        "Producible (no charm guard on either side) but the add-on's admin plane is dead on arrival: tenant-service authenticates its REST/gRPC admin API by PARSING the bearer token as a JWT against hydra's JWKS/issuer — the only verifier is JWTVerifier around oidc.IDTokenVerifier.Verify, and `introspect` appears nowhere in the repo — so with hydra's jwt_access_tokens=false every client-credentials token it mints is `ory_at_…` and every admin call answers 401 'invalid token'. Only the credential-less gRPC LookupTenants keeps working, so users can still log in while nobody can create, list or delete a tenant. The test plane seeds tenants through that admin API, so the shape cannot be seeded, let alone tested (nightly red on every such row 2026-08-27 → 2026-09-08). Retired from generation as a documented-unsupported shape (the harness reaction the upstreamFindings entry names); delete this constraint when the service validates opaque tokens by introspection.",
      evidence:
        "canonical/tenant-service@cc6ae33 (v0.3.1) pkg/authentication/verifier.go:19-36 (JWTVerifier.VerifyToken → oidc.IDTokenVerifier.Verify), cmd/serve.go:117-147 (JWT verifier or noop, nothing else); canonical/hydra-operator@f7e000b charmcraft.yaml:95-99 `jwt_access_tokens`; measured: tests/browser/seeder/seed.ts `cleanupOwnedTenants` → `failed to list tenants: 401 {\"message\":\"invalid token\",\"status\":401}` on mx-l1m0v1wsp2t1h0u0ao, mx-l1m1v1wnp1t1h1u1ao, mx-l0m0v0wnp2t1h1u1ao (nightly 2026-09-08, run 34176722469)",
    },
    {
      id: "opaque-tokens-lock-out-hook-admin",
      forbid: { access_token: "opaque", hook_service: "present" },
      reason:
        "Same defect, second service. hydra's token hook itself is unaffected (it authenticates with the shared API_TOKEN, and the preflight's hook counter advances on opaque rows), but hook-service's Group/Authz management API — the only writer of group membership, hence of the `groups` claim — is JWT-only like tenant-service's: with jwt_access_tokens=false the admin client's token is refused with 401 'invalid token', no group can be seeded, and the claim the row exists to observe can never be populated. The charm renders exactly the same verifier inputs (AUTHENTICATION_ISSUER/JWKS_URL from `authn_*` or the oauth relation; no introspection option exists). Retired from generation as documented-unsupported; delete when the service validates opaque tokens by introspection.",
      evidence:
        "canonical/hook-service@295273b pkg/authentication/verifier.go:18-35 (JWTVerifier.VerifyToken → oidc.IDTokenVerifier.Verify), cmd/serve.go:147-176 (JWT verifier or noop); canonical/hook-service-operator@186d47f src/integrations.py:295-310 (OAuthProviderData renders AUTHENTICATION_ISSUER/JWKS_URL only), charmcraft.yaml `authn_issuer`/`authn_jwks_url`; measured: tests/browser/seeder/seed.ts `seed hook-service groups: hook-service GET /groups: 401 invalid token` on mx-l1m1v1wnp0t0h1u0ao (nightly 2026-09-08, run 34176722469)",
    },
  ],
