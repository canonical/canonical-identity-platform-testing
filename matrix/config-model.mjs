...
export const model = {
  ...
  seeds: [
    ...
    },
    {
      ...
      reason:
        ...
        "The target is deployed by canonical/cd-identity-core-infrastructure#61 (merged 2026-08-21): orange-core.tfvars pins external_hostname = iam.orange.canonical.com, and orange-iam/main.tf sources iam-bundle-integration?ref=istio with no tenant-service, hook-service or user-verification-service — which independently confirms the three add-on dims, and, with kratos/hydra/login_ui config left at the charm defaults, local_idp=on, mfa=enforced, verification=off and access_token=jwt. providers is the one dim where the declaration and the deployment DISAGREE: that same file sets enable_kratos_external_idp_integrator = true with provider google / provider_id google_canonical, yet no oidc node reaches any login flow. The row declares what the deployment SERVES (providers=0) — declaring 1 would fail this row's own preflight — and the discrepancy is filed under upstreamFindings.",
      dims: {
        local_idp: "on",
        mfa: "enforced",
        verification: "off",
  ...
  upstreamFindings: [
    ...
    "CONFIRMED LIVE 2026-08-27, iam.orange.canonical.com: THE SETTINGS AAL2 STEP-UP IS BROKEN because kratos's public base_url is the CLUSTER-INTERNAL service address. Reaching any /ui/manage_* page without a satisfying AAL2 session makes the SPA call /self-service/settings/browser; kratos answers with a redirect to /self-service/login/browser?aal=aal2&return_to=http%3A%2F%2Fkratos.orange-iam.svc.cluster.local%3A4433%2Fself-service%2Fsettings%2Fbrowser%3F… — a return_to minted from its own base_url — and then REFUSES that very URL: HTTP 400 {\"error\":{\"id\":\"self_service_flow_return_to_forbidden\",\"reason\":\"Requested return_to URL \\\"http://kratos.orange-iam.svc.cluster.local:4433/…\\\" is not allowed.\"}} rendered as raw JSON in the user's browser. Users with a fresh password+TOTP session never hit it (settings load directly), which is why the suite's settings scenarios pass; any stale/AAL1 session does. The kratos flows' request_url has shown the same internal base since first contact (2026-08-26). This is the C-16 pattern on the DEPLOYMENT side — serve.public.base_url must be the ingress, not the svc address; orange-iam relates kratos:public-route to the istio offer (canonical/cd-identity-core-infrastructure#61 integrations.tf), so either that relation does not feed base_url on the istio charm track or it is rendered wrong. Needs juju config kratos / the rendered kratos.yaml on orange-iam.",
    "OPEN 2026-08-26, iam.orange.canonical.com: the deployment DECLARES one external IdP and offers none. canonical/cd-identity-core-infrastructure#61 orange-iam/main.tf sets enable_kratos_external_idp_integrator = true with config provider=google, provider_id=google_canonical, yet no login or registration flow carries an oidc provider node and /api/v0/app-config's flags omit account_linking — and kratos itself renders those flows, so kratos has no provider configured and the integrator's relation data is not reaching it. Unresolvable from outside (no substrate access on this lane): next step is `juju status -m orange-iam` for the kratos-external-idp-integrator app plus its kratos-external-idp relation. Until then the matrix row declares providers=0, matching what the deployment serves.",
